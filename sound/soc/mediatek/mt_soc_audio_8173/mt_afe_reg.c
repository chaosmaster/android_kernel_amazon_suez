...
static void *infra_sram_address;
...
static phys_addr_t infra_sram_phy_address = INFRA_SRAM_PHY_BASE;
static uint32_t afe_sram_size = AFE_INTERNAL_SRAM_SIZE;
static uint32_t infra_sram_size;
...
int mt_afe_reg_remap(void *dev)
{
	...
	/* audio infra SRAM base */
	ret = of_address_to_resource(pdev->of_node, 2, &res);
	if (ret) {
		pr_warn("%s infra sram : of_address_to_resource#1 fail %d\n", __func__, ret);
	} else {
		infra_sram_address = ioremap_nocache(res.start, resource_size(&res));
		if (!infra_sram_address) {
			pr_err("%s ioremap_nocache#1 addr:0x%llx size:0x%llx fail\n",
			       __func__, (unsigned long long)res.start,
			       (unsigned long long)resource_size(&res));
		} else {
			infra_sram_phy_address = res.start;
			infra_sram_size = resource_size(&res);
		}
	}
#endif
 ...
 exit:
	...
	apmixedsys_base_address = ioremap_nocache(APMIXEDSYS_BASE, 0x1000);
#ifdef AUDIO_USE_INFRA_SRAM
	infra_sram_address = ioremap_nocache(INFRA_SRAM_PHY_BASE, INFRA_SRAM_SIZE);
#endif
	return 0;
#endif
}
...
#ifdef AUDIO_USE_INFRA_SRAM
	if (infra_sram_address) {
		iounmap(infra_sram_address);
		infra_sram_address = NULL;
	}
#endif
	if (topckgen_base_address) {
...
void *mt_afe_get_infra_sram_base_ptr(void)
{
	return infra_sram_address;
}

phys_addr_t mt_afe_get_infra_sram_phy_addr(void)
{
	return infra_sram_phy_address;
}

uint32_t mt_afe_get_infra_sram_size(void)
{
	return infra_sram_size;
}
uint32_t mt_afe_get_infra_capture_sram_size(void)
{
	return UL1_SHARED_SRAM_BUFFER_SIZE;
}
uint32_t mt_afe_get_infra_playback_sram_size(void)
{
	return DL1_SHARED_SRAM_BUFFER_SIZE;
}
