...
static const char *const mt_I2S0dl1_volume_key_switch[] = {
...
static int Audio_I2S0dl1_volkey_switch_get
...
static int Audio_I2S0dl1_volkey_switch_set
		(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
#ifdef CONFIG_KPD_VOLUME_KEY_SWAP
...
static const struct snd_kcontrol_new mt_I2S0dl1_volkey_controls[] = {
	...
	Audio_I2S0dl1_volkey_switch_get,
	Audio_I2S0dl1_volkey_switch_set),
};
...
static int dl1_i2s0_lr_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl1_priv *priv = dev_get_drvdata(component->dev);
	ucontrol->value.integer.value[0] = priv->lr_switch;
	return 0;
}

static int dl1_i2s0_lr_switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl1_priv *priv = dev_get_drvdata(component->dev);
	if ((priv->prepared) && (priv->lr_switch != ucontrol->value.integer.value[0])) {
		if (ucontrol->value.integer.value[0] == LR_NORMAL) {
			mt_afe_set_connection(INTER_DISCONNECT, INTER_CONN_I05, INTER_CONN_O04);
			mt_afe_set_connection(INTER_DISCONNECT, INTER_CONN_I06, INTER_CONN_O03);
			...
			mt_afe_set_connection(INTER_CONNECT, INTER_CONN_I05, INTER_CONN_O04);
			mt_afe_set_connection(INTER_CONNECT, INTER_CONN_I06, INTER_CONN_O03);
		}
	}
				 ...
				 struct snd_pcm_hw_params *hw_params)
{
	...
	if (buffer_size > mt_afe_get_infra_playback_sram_size()) {
	...
	if (priv->enable_sram) {
		...
		substream->runtime->dma_area = (unsigned char *)mt_afe_get_infra_sram_base_ptr() + mt_afe_get_infra_capture_sram_size();
		substream->runtime->dma_addr = mt_afe_get_infra_sram_phy_addr() + mt_afe_get_infra_capture_sram_size();
	} else {
...
static const struct snd_kcontrol_new mt_pcm_dl1_controls[] = {
	...
	SOC_ENUM_EXT("DL1_LR_SWITCH", mt_pcm_dl1_control_enum[2],
		dl1_i2s0_lr_switch_get, dl1_i2s0_lr_switch_set),
};
