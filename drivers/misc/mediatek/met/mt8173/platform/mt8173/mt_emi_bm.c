...
/*#include <asm/io.h>*/
#include <mt-plat/sync_write.h> /*add */
...
static const char of_dramc_desc[] = "mediatek,DRAMC0_NAO";

#define	ADDR_DRAMC1	((unsigned long)BaseAddrDRAMC1)
static void __iomem *BaseAddrDRAMC1;
static const char of_dramc1_desc[] = "mediatek,DRAMC1_NAO";
...
int MET_BM_Init(void)
{
  ...
  printk("km input MET_BM__Init...\n");
	node = of_find_compatible_node(NULL, NULL, of_emi_desc);
	printk("km MET_BM_Init node =%p\n",node);
	if (node == NULL) {
		return -1;
	}
	BaseAddrEMI = of_iomap(node, 0);
	of_node_put(node);
	printk("km MET_BM_Init BaseAddrEMI =%p \n",BaseAddrEMI);
	if (BaseAddrEMI == 0) {
		return -2;
	}
	...
	node = of_find_compatible_node(NULL, NULL, of_dramc1_desc);
	if (node == NULL) {
		iounmap(BaseAddrDRAMC);
		iounmap(BaseAddrEMI);
		return -5;
	}
	BaseAddrDRAMC1 = of_iomap(node, 0);
	of_node_put(node);
	if (BaseAddrDRAMC1 == 0) {
		iounmap(BaseAddrDRAMC);
		iounmap(BaseAddrEMI);
...
unsigned int MET_DRAMC_GetPageHitCount1(DRAMC_Cnt_Type CountType)
{
	...
	switch (CountType) {
	case DRAMC_R2R:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2R_PAGE_HIT));
		break;
	case DRAMC_R2W:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2W_PAGE_HIT));
		break;
	case DRAMC_W2R:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2R_PAGE_HIT));
		break;
	case DRAMC_W2W:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2W_PAGE_HIT));
		break;
	case DRAMC_ALL:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2R_PAGE_HIT)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2W_PAGE_HIT)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2R_PAGE_HIT)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2W_PAGE_HIT));
		break;
	default:
		return BM_ERR_WRONG_REQ;
	}
...
unsigned int MET_DRAMC_GetPageMissCount1(DRAMC_Cnt_Type CountType)
{
	...
	switch (CountType) {
	case DRAMC_R2R:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2R_PAGE_MISS));
		break;
	case DRAMC_R2W:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2W_PAGE_MISS));
		break;
	case DRAMC_W2R:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2R_PAGE_MISS));
		break;
	case DRAMC_W2W:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2W_PAGE_MISS));
		break;
	case DRAMC_ALL:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2R_PAGE_MISS)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2W_PAGE_MISS)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2R_PAGE_MISS)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2W_PAGE_MISS));
		break;
	default:
		return BM_ERR_WRONG_REQ;
	}
...
unsigned int MET_DRAMC_GetInterbankCount1(DRAMC_Cnt_Type CountType)
{
	...
	switch (CountType) {
	case DRAMC_R2R:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2R_INTERBANK));
		break;
	case DRAMC_R2W:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2W_INTERBANK));
		break;
	case DRAMC_W2R:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2R_INTERBANK));
		break;
	case DRAMC_W2W:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2W_INTERBANK));
		break;
	case DRAMC_ALL:
		iCount = readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2R_INTERBANK)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_R2W_INTERBANK)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2R_INTERBANK)) +
		    readl(IOMEM(ADDR_DRAMC1 + DRAMC_W2W_INTERBANK));
		break;
	default:
		return BM_ERR_WRONG_REQ;
	}
...
{
	return readl(IOMEM(ADDR_DRAMC1 + DRAMC_IDLE_COUNT));
}
