...
static const char help[] =
    "  --cci400=src:evt                      src: S0~S4 or M0~M2, evt:0x00~0x14 for S*, 0x00~0x11 for M*\n";
...
static void cci400(unsigned long long stamp, int cpu)
{
	...
	case 1:
		MET_PRINTK(CCI_FMT1, CCI_VAL1);
		break;
#endif
	case 2:
		MET_PRINTK(CCI_FMT2, CCI_VAL2);
		break;
	case 3:
		MET_PRINTK(CCI_FMT3, CCI_VAL3);
		break;
	case 4:
		MET_PRINTK(CCI_FMT4, CCI_VAL4);
		break;
#ifdef PROBE_CCI400_CYCLES
	case 5:
		MET_PRINTK(CCI_FMT5, CCI_VAL5);
		break;
#endif
	}
}
...
#ifdef PROBE_CCI400_CYCLES
static const char header[] = "met-info [000] 0.0: ms_ud_sys_header: cci400,CYCLES";
#else
static const char header[] = "met-info [000] 0.0: ms_ud_sys_header: cci400";
#endif

char *met_cci400_src_name[] = { "S0", "S1", "S2", "S3", "S4", "M0", "M1", "M2" };

static inline int reset_driver_stat(void)
{
	met_cci400.mode = 0;
	used_count = 0;
	start = 0;
...
static int cci400_print_header(char *buf, int len)
{
	...
	for (i = 0; i < used_count; i++) {
		size = snprintf(buf, PAGE_SIZE, ",%s:0x%02X",
				met_cci400_src_name[ccipmu[i].evt_sel.interface], ccipmu[i].evt_sel.event);
		total_size += size;
		buf += size;
	}
#ifdef PROBE_CCI400_CYCLES
