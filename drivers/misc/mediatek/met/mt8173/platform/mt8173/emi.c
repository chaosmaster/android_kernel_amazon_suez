...
static unsigned int (*mt_get_emi_freq_symbol) (void);
...
static struct kobj_attribute rwtype_attr = __ATTR(rwtype,
						  0664,
						  rwtype_show,
						  rwtype_store);
...
static ssize_t emi_clock_rate_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (mt_get_emi_freq_symbol)
		i = snprintf(buf, PAGE_SIZE, "%d\n", mt_get_emi_freq_symbol());
	else
		i = snprintf(buf, PAGE_SIZE, "%d\n", 0);
...
static ssize_t dcm_ctrl_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
			      ...
			      struct kobj_attribute *attr, const char *buf, size_t n)
{
	if (!strncmp(buf, "OFF", strlen("OFF")))
		...
		printk(KERN_DEBUG"invalid event\n");

	return n;
}

static ssize_t test_cqdma_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	i = snprintf(buf, PAGE_SIZE, "%d\n", times);
	return i;
				...
				struct kobj_attribute *attr, const char *buf, size_t n)
{
	...
	/* dma_alloc */
	src_addr_v = dma_alloc_coherent(NULL, PAGE_SIZE, &src_addr_p, GFP_KERNEL);
	if (src_addr_v == NULL)
		return -ENOMEM;

	dst_addr_v = dma_alloc_coherent(NULL, PAGE_SIZE, &dst_addr_p, GFP_KERNEL);
	if (dst_addr_v == NULL)
		return -ENOMEM;
	...
	/* dma_free */
	if (src_addr_v != NULL)
		dma_free_coherent(NULL, PAGE_SIZE, src_addr_v, src_addr_p);
	if (dst_addr_v != NULL)
		dma_free_coherent(NULL, PAGE_SIZE, dst_addr_v, dst_addr_p);
	return n;
}

static ssize_t test_apmcu_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	i = snprintf(buf, PAGE_SIZE, "%d\n", times);
	return i;
...
static ssize_t ttype_enable_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
...
static ssize_t bw_limiter_enable_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
...
static ssize_t ttype_master_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype_master) {
...
static ssize_t ttype_busid_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
...
static ssize_t ttype1_nbeat_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype1_nbeat) {
...
static ssize_t ttype1_nbyte_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype1_nbyte) {
...
static ssize_t ttype1_burst_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype1_burst) {
...
static ssize_t ttype2_nbeat_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype2_nbeat) {
	case BM_TRANS_TYPE_1BEAT:
...
static ssize_t ttype2_nbyte_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype2_nbyte) {
	case BM_TRANS_TYPE_1Byte:
...
static ssize_t ttype2_burst_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype2_burst) {
	case BM_TRANS_TYPE_BURST_INCR:
...
static ssize_t ttype3_nbeat_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype3_nbeat) {
	case BM_TRANS_TYPE_1BEAT:
...
static ssize_t ttype3_nbyte_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype3_nbyte) {
	case BM_TRANS_TYPE_1Byte:
...
static ssize_t ttype3_burst_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype3_burst) {
	case BM_TRANS_TYPE_BURST_INCR:
...
static ssize_t ttype4_nbeat_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype4_nbeat) {
	case BM_TRANS_TYPE_1BEAT:
...
static ssize_t ttype4_nbyte_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype4_nbyte) {
	case BM_TRANS_TYPE_1Byte:
...
static ssize_t ttype4_burst_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype4_burst) {
	case BM_TRANS_TYPE_BURST_INCR:
...
static ssize_t ttype5_nbeat_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype5_nbeat) {
	case BM_TRANS_TYPE_1BEAT:
...
static ssize_t ttype5_nbyte_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype5_nbyte) {
	case BM_TRANS_TYPE_1Byte:
...
static ssize_t ttype5_burst_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int i;
	switch (ttype5_burst) {
	case BM_TRANS_TYPE_BURST_INCR:
...
static void emi_init(void)
{
	...
	if (ttype_enable == BM_TTYPE_ENABLE) {
		...
		MET_BM_SetMonitorCounter(17,
					 ttype_master, ttype1_nbeat | ttype1_nbyte | ttype1_burst);
		MET_BM_SetIDSelect(17, ttype_busid, 1);

		/* TTYPE18 */
		MET_BM_SetMonitorCounter(18,
					 ttype_master, ttype2_nbeat | ttype2_nbyte | ttype2_burst);
		MET_BM_SetIDSelect(18, ttype_busid, 1);

		/* TTYPE19 */
		MET_BM_SetMonitorCounter(19,
					 ttype_master, ttype3_nbeat | ttype3_nbyte | ttype3_burst);
		MET_BM_SetIDSelect(19, ttype_busid, 1);

		/* TTYPE20 */
		MET_BM_SetMonitorCounter(20,
					 ttype_master, ttype4_nbeat | ttype4_nbyte | ttype4_burst);
		MET_BM_SetIDSelect(20, ttype_busid, 1);

		/* TTYPE21 */
		MET_BM_SetMonitorCounter(21,
					 ttype_master, ttype5_nbeat | ttype5_nbyte | ttype5_burst);
...
static unsigned int emi_bw_limiter(unsigned int *array)
{
	...
	/* Clk + ARB* */
	if (mt_get_emi_freq_symbol)
		array[++idx] = mt_get_emi_freq_symbol();
	else
		array[++idx] = 0;
	/* printk("===freq=%d\n", array[idx]); */
	array[++idx] = MET_EMI_GetARBA();
	/* printk("===arba=%x\n", array[idx]); */
	/* emi_value[++idx] = MET_EMI_GetARBB(); */
	array[++idx] = 0;
	/* printk("===arbb=%x\n", array[idx]); */
...
static unsigned int emi_polling(unsigned int *emi_value)
{
	...
	/* Reset */
	for (y = 0; y < 4; y++) {
		buffer[y] = MET_BM_GetWordCount(y+1);
		if (buffer[y] > emi_value[y+1])
			break;
	}
	if (y < 4) {
		for (x = 0; x < 100; x++) {
		...
		if (x == 100)
			MET_PRINTK("Reset failed!!!\n");
	}
Finish:
#if 0
	/* Get Word Count */
	for (i = 0; i < 4; i++) {
		buffer[i] = MET_BM_GetWordCount(i+1);
	}
	MET_PRINTK("[EMI_WSCT] %d,%d,%d,%d\n",
			buffer[0],
			buffer[1],
			buffer[2],
			buffer[3]);
	MET_PRINTK("[EMI_CONM] 0x%x\n", MET_BM_GetEmiDcm());
	MET_PRINTK("[INFRA_DCMCTL] 0x%x\n", MET_BM_GetInfraDcm());
#endif
	return j + 1;
}
...
static int met_emi_create(struct kobject *parent)
{
	...
	mt_get_emi_freq_symbol = symbol_get(mt_get_emi_freq);
	if (!mt_get_emi_freq_symbol)
		pr_err("Can't find mt_get_emi_freq()\n");
...
static void met_emi_polling(unsigned long long stamp, int cpu)
{
	...
	if (do_emi()) {
		...
		if (count) {
			emi_value[1] = emi_value[1] - emi_value[2] - emi_value[3] - emi_value[4];
			...
			if ((bw_limiter_enable == BM_BW_LIMITER_ENABLE) &&
				...
				count = emi_bw_limiter(bw_limiter);
				if (count)
					ms_bw_limiter(stamp, NIDX_BL, bw_limiter);
...
"MDHW_TRANS,MM_TRANS,GPU_TRANS,GPU_1_TRANS,"
"BACT,BSCT,BCNT,"
"PageHit,PageMiss,InterBank,Idle," "PageHit_1,PageMiss_1,InterBank_1,Idle_1\n";
static const char header_ttype[] =
"met-info [000] 0.0: ms_ud_sys_header: ms_ttype,"
"ttyp1,ttyp2,ttyp3,ttyp4,ttyp5," "x,x,x,x,x\n";
static const char header_bw_limiter[] =
"met-info [000] 0.0: met_bw_limiter_header: CLK," "ARBA,ARBB,ARBC,ARBD,ARBE,ARBF,ARBG,ARBH\n";
...
static int emi_print_header(char *buf, int len)
{
	...
	if (mt_get_emi_freq_symbol)
		ret = snprintf(buf, PAGE_SIZE, header, mt_get_emi_freq_symbol());
	else
		ret = snprintf(buf, PAGE_SIZE, header, 0);
	if (ttype_enable == BM_TTYPE_ENABLE)
