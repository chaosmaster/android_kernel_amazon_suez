...
static ssize_t kptr_restrict_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t kptr_restrict_store(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t count);
static DEVICE_ATTR(kptr_restrict, 0664, kptr_restrict_show, kptr_restrict_store);
...
int fs_reg(void)
{
	...
	ret = device_create_file(met_device.this_device, &dev_attr_kptr_restrict);
	if (ret != 0) {
		pr_err("can not create device file: kptr_restrict\n");
		return ret;
	}
...
#endif
	device_remove_file(met_device.this_device, &dev_attr_kptr_restrict);
