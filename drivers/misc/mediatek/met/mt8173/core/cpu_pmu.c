...
static ssize_t count_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
			   size_t n)
{
	return -EINVAL;
}
...
static ssize_t event_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
			   size_t n)
{
	struct met_pmu *p = lookup_pmu(kobj);
...
static ssize_t freq_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
			  size_t n)
{
	struct met_pmu *p = lookup_pmu(kobj);
	if (p != NULL) {
	...
	for (i = 0; i < nr_counters; i++) {
		...
		if (cpu_pmu->get_event_desc && 0 == cpu_pmu->get_event_desc(i, pmu[i].event, name)) {
			if (first) {
				...
				ret +=
				    snprintf(buf + ret, PAGE_SIZE, ",0x%x:%s", pmu[i].event, name);
			}
		} else {
			if (first) {
