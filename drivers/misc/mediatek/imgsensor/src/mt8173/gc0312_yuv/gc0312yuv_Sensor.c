...
void GC0312_Sensor_Init(void)
{
	...
	GC0312_write_cmos_sensor(0x59, 0x00);  /*02  20160406*/
...
UINT32 GC0312GetSensorID(UINT32 *sensorID)
{
	...
	SENSORDB("FFC, GC0312GetSensorID start\n");

	/* check FFC id pin */
	ffc_id = mtkcam_gpio_read_id(1);

	if (ffc_id == 0xFF)
		SENSORDB("FFC, read sensor id pin fail!\n");

	SENSORDB("FFC, read sensor id pin = %d\n", ffc_id);
