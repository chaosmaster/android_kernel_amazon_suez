...
struct mtk_i2c {
	...
	/* en pin control */
	void __iomem *gpiobase;		/* gpio base addr for ea pin control */
	bool enable_ea_pin;		/* enable ea pin */
	u32 regs[4];
...
static void mtk_i2c_init_hw(struct mtk_i2c *i2c)
{
	...
	if (i2c->enable_ea_pin)
		writew((readw(i2c->gpiobase + i2c->regs[2]))|i2c->regs[3], (i2c->gpiobase + i2c->regs[2]));
			    ...
			    unsigned int *clk_src_div)
{
	...
	i2c->enable_ea_pin = of_property_read_bool(np, "mediatek,enable-ea");
	if (i2c->enable_ea_pin) {
		of_property_read_u32_array(np, "ea-ctrl-addr", i2c->regs, 4);
		i2c->gpiobase = ioremap_nocache(i2c->regs[0], i2c->regs[1]);
	}
