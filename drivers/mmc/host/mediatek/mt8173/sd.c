...
static bool log_dumping = false;
static u32 log_pos_idx = 0;
...
void msdc_dump_register(struct msdc_host *host)
{
	...
	pr_err("SDIO TOP_DAT_TUNE_CTRL1=0x%.8x\n",
			top_sdr_read32(host->top + MSDC_TOP_DAT_TUNE_CTRL1));
	pr_err("SDIO TOP_DAT_TUNE_CTRL2=0x%.8x\n",
			top_sdr_read32(host->top + MSDC_TOP_DAT_TUNE_CTRL2));
	pr_err("SDIO TOP_DAT_TUNE_CTRL3=0x%.8x\n",
			top_sdr_read32(host->top + MSDC_TOP_DAT_TUNE_CTRL3));
	return;
}
...
void msdc_sdio_set_long_timing_delay_by_freq(struct msdc_host *host, u32 clock)
{
	...
	if (clock >= 200000000) {
		...
		top_sdr_set_field(host->top + MSDC_TOP_DAT_TUNE_CTRL1,
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (host->hw->host_function == MSDC_SDIO
		&& hz >= 100 * 1000 * 1000 && sdio_online_tune_fail)
		hz = 50 * 1000 * 1000;
#endif
...
static struct msdc_host *sdio_host = NULL;
...
static int autok_wcrc_int_dly_sel(struct AUTOK_REF_INFO *bdinf,
		unsigned int *pad_dly,
		unsigned int ckgen_base)
{
	return autok_optimised_dly_sel(bdinf->bound_int, pad_dly, 0, NULL,
			ckgen_base, bdinf->bound_cycle, 0);
}
		...
		u32 *value, int rw)
{
	...
	if (rw == AUTOK_READ) {
		if (top_reg_access)
			top_sdr_get_field(reg, field, value);
		else
			sdr_get_field(reg, field, *value);
	} else if (rw == AUTOK_WRITE)	{
		if (top_reg_access)
			top_sdr_set_field(reg, field, *value);
		else
...
static int autok_path_sel(struct msdc_host *host, bool need_autok)
{
	...
	if (need_autok) {
		...
		/*data tune mode select*/
		top_sdr_clr_bits(host->top + MSDC_TOP_DAT_TUNE_CTRL1, MSDC_TOP_SDIO_DATA_TUNE_SEL);
...
int sdio_stop_transfer(void)
{
	...
	/* pr_err("[AUTOK] stop transfer. c_vol:%d\n",
...
int sdio_start_ot_transfer(void)
{
	...
	pr_err("[AUTOK] start transfer. c_vol:%d, hz:%d, done:%d\n",
