...
void mtk_wdt_mode_config(bool dual_mode_en, bool irq, bool ext_en, bool ext_pol, bool wdt_en)
{
	...
	/* Bit5: irq_lvl_en: this bit should always set to 0 to select irq type to edge, 1 is level */
	tmp &= ~MTK_WDT_MODE_IN_DIS;
...
#endif
	udelay(100);
	pr_err("mtk_wdt_probe : done WDT_MODE(%x),MTK_WDT_NONRST_REG(%x)\n",
