...
static int dt_to_keycombo_cfg(struct platform_device *pdev, struct keycombo_platform_data **pcfg)
{
	...
	for (idx  = 0; idx < down_size; idx++) {
		...
		strncat(keys_cfg.panic_msg + strlen(keys_cfg.panic_msg), name, PANIC_MSG_SIZE - strlen(keys_cfg.panic_msg));
		strncat(keys_cfg.panic_msg + strlen(keys_cfg.panic_msg), " ", PANIC_MSG_SIZE - strlen(keys_cfg.panic_msg));
	}
...
static int keypanic_probe(struct platform_device *pdev)
{
	...
	keys_cfg.combo_cfg_size = dt_to_keycombo_cfg(pdev, &keys_cfg.pcombo_cfg);
	...
	ret = platform_device_add_data(keys_cfg.pdev_keycombo, keys_cfg.pcombo_cfg, keys_cfg.combo_cfg_size);
	if (ret)
		goto error;
