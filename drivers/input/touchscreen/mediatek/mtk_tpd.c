...
#ifdef CONFIG_COMPAT
#define COMPAT_TPD_GET_FILTER_PARA	_IOWR( \
					TOUCH_IOC_MAGIC, 2, struct tpd_filter_t)
#endif
...
void tpd_get_dts_info(void)
{
	...
	if (node1) {
		of_property_read_u32(node1, "tpd-key-dim-local",
				     &tpd_dts_data.touch_max_num);
		of_property_read_u32(node1, "use-tpd-button",
		...
		if (tpd_dts_data.use_tpd_button) {
			of_property_read_u32(node1, "tpd-key-num",
					     &tpd_dts_data.tpd_key_num);
			...
			of_property_read_u32_array(node1, "tpd-key-dim-local",
						   key_dim_local,
						   ARRAY_SIZE(key_dim_local));
			memcpy(tpd_dts_data.tpd_key_dim_local, key_dim_local,
			       sizeof(key_dim_local));
			for (i = 0; i < 4; i++) {
				pr_info("[tpd]key[%d].key_x = %d\n", i,
					tpd_dts_data.tpd_key_dim_local[i].
					key_x);
		...
		memcpy(&tpd_filter, &tpd_dts_data.touch_filter,
		       sizeof(tpd_filter));
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	if (IS_ERR(pins_default)) {
		...
		dev_err(&pdev->dev,
			"fwq Cannot find touch pinctrl default %d!\n", ret);
	}
	...
	if (IS_ERR(eint_as_int)) {
		...
		dev_err(&pdev->dev,
			"fwq Cannot find touch pinctrl state_eint_as_int!\n");
		return ret;
	}
	...
	if (IS_ERR(eint_output0)) {
		...
		dev_err(&pdev->dev,
			"fwq Cannot find touch pinctrl state_eint_output0!\n");
		return ret;
	}
	...
	if (IS_ERR(eint_output1)) {
		...
		dev_err(&pdev->dev,
			"fwq Cannot find touch pinctrl state_eint_output1!\n");
		return ret;
	}
	...
	if (IS_ERR(rst_output0)) {
		...
		dev_err(&pdev->dev,
			"fwq Cannot find touch pinctrl state_rst_output0!\n");
		return ret;
	}
	...
	if (IS_ERR(rst_output1)) {
		...
		dev_err(&pdev->dev,
			"fwq Cannot find touch pinctrl state_rst_output1!\n");
		return ret;
	}
...
#ifdef CONFIG_COMPAT
static long tpd_compat_ioctl(struct file *file, unsigned int cmd,
			     unsigned long arg)
{
	long ret;
			       ...
			       unsigned long arg)
{
	...
	case TPD_GET_VELOCITY_CUSTOM_X:
		...
		if (copy_to_user
		    (data, &tpd_v_magnify_x, sizeof(tpd_v_magnify_x))) {
			err = -EFAULT;
			break;
		}
	...
	case TPD_GET_VELOCITY_CUSTOM_Y:
		...
		if (copy_to_user
		    (data, &tpd_v_magnify_y, sizeof(tpd_v_magnify_y))) {
			err = -EFAULT;
			break;
		}
	...
	case TPD_GET_FILTER_PARA:
		...
		if (data == NULL) {
			err = -EINVAL;
			pr_err
			("tpd:TPD_GET_FILTER_PARA IOCTL:data is null\n");
			break;
		}

		if (copy_to_user(data,
			&tpd_filter, sizeof(struct tpd_filter_t))) {
			pr_err
			  ("tpd:TPD_GET_FILTER_PARA IOCTL:copy data error\n");
			err = -EFAULT;
			break;
		}
		break;
				    ...
				    unsigned long event, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		...
		if (g_tpd_drv && tpd_suspend_flag) {
			err =
			    queue_work(touch_resume_workqueue,
				       &touch_resume_work);
			if (!err) {
				TPD_DMESG
				    ("start touch_resume_workqueue failed\n");
				return err;
			}
		}
		break;
...
static int tpd_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_MTK_FB
		/* Fix build errors, as some projects cannot
		* support these apis while bring up */
...
#ifdef CONFIG_MTK_FB
	/*Fix build errors, as some projects cannot support
	 * these apis while bring up */
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			...
			if (tpd_load_status == 1) {
				TPD_DMESG
				    ("[mtk-tpd]tpd_probe, tpd_driver_name=%s\n",
				     tpd_driver_list[i].tpd_device_name);
				g_tpd_drv = &tpd_driver_list[i];
				break;
			}
	...
	if (g_tpd_drv == NULL) {
		if (tpd_driver_list[0].tpd_device_name != NULL) {
			...
			TPD_DMESG
			("[mtk-tpd]cap & Generic touch both are not loaded\n");
			return 0;
		}
	}
	...
	if (touch_type == 1) {
		...
		input_set_abs_params(tpd->dev, ABS_MT_POSITION_X, 0, TPD_RES_X,
				     0, 0);
...
#else
		input_set_abs_params(tpd->dev, ABS_MT_TOUCH_MAJOR, 0, 100, 0,
				     0);
		input_set_abs_params(tpd->dev, ABS_MT_TOUCH_MINOR, 0, 100, 0,
				     0);
#endif /* CONFIG_MTK_S3320 */
